...
export async function load(scene: Scene): Promise<Model> {
  ...
  if (!loaded.meshes[0]) {
    throw new Error("No mesh found when loading ./assets/Warrior.glb.");
  }
  ...
  return {
    ...
    animations: {
      ...
      Sword_Attack: loaded.animationGroups.find(ac => ac.name === "Sword_Attack")!,
